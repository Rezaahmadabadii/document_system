    ...
    <div class="documents-layout">
        <div class="right-panel-panel">
            <div class="main-panel-card">
                ...
                <div id="userFormPanel" class="user-form-panel">
                    ...
                    <div class="form-section" style="border-top: 1px solid #eef2f5; margin-top: 0;"><div class="form-group"><label>گزارش / یادداشت</label><textarea id="doc_description" rows="3" placeholder="هرگونه توضیح یا گزارش اضافی..."></textarea></div><button class="btn-green" id="submitDescriptionBtn">✏️ ثبت توضیح</button></div>
                </div>
                
                <div id="userSearchPanel" class="user-search-panel">
                    <div style="padding: 14px 16px;">
                        <div class="form-group"><label>شماره سند</label><input type="text" id="filter_number" placeholder="جستجو..." oninput="autoSearchDocuments()"></div>
                        <div class="form-group"><label>تاریخ سند</label><input type="text" id="filter_date" placeholder="1404/01/01" oninput="autoSearchDocuments()"></div>
                        <div class="form-group"><label>شرکت</label><select id="filter_company" onchange="autoSearchDocuments()"><option value="">همه شرکت‌ها</option><?php foreach($companies as $c): ?><option value="<?php echo $c['id']; ?>"><?php echo htmlspecialchars($c['name']); ?></option><?php endforeach; ?></select></div>
                        <div class="form-group"><label>تاریخ تحویل</label><input type="text" id="filter_delivery" placeholder="1404/01/01" oninput="autoSearchDocuments()"></div>
        ...
        <div class="left-panel-list">
            <div id="leftContentArea">
                ...
                <?php else: ?>
                <div id="userDocsContainer"><div class="docs-list" id="documents_list"><div class="empty-state">در حال بارگذاری...</div></div></div>
                <?php endif; ?>
            </div>
        </div>
    </div>
...
function autoSearchDocuments() { 
    ...
    searchTimeout = setTimeout(() => { if (typeof loadSearchDocuments === 'function') loadSearchDocuments(); }, 400); 
}
    ...
    async function loadLastDeliveryDate() {
        try {
            ...
            if (data.success && data.last_date) {
                ...
                await checkLockStatus(currentDeliveryDate);
            } else {
                let today = '<?php echo $today; ?>';
                currentDeliveryDate = today;
                document.getElementById('delivery_date').value = today;
